...
import ru.lischenko_dev.fastmessenger.vkapi.models.CommentList;
import ru.lischenko_dev.fastmessenger.vkapi.models.Counters;
import ru.lischenko_dev.fastmessenger.vkapi.models.Country;
import ru.lischenko_dev.fastmessenger.vkapi.models.FriendsList;
import ru.lischenko_dev.fastmessenger.vkapi.models.GroupBanItem;
import ru.lischenko_dev.fastmessenger.vkapi.models.GroupTopic;
import ru.lischenko_dev.fastmessenger.vkapi.models.NewsFeed;
import ru.lischenko_dev.fastmessenger.vkapi.models.PhotoTag;
import ru.lischenko_dev.fastmessenger.vkapi.models.SearchDialogItem;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKAlbum;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKApp;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKAttachment;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKAudio;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKChat;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKComment;
...
import ru.lischenko_dev.fastmessenger.vkapi.models.VKLink;
...
import ru.lischenko_dev.fastmessenger.vkapi.models.VKNote;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKNotifications;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKPhoto;
import ru.lischenko_dev.fastmessenger.vkapi.models.VKPoll;
...
public class Api {
    ...
    public static String getOnlineApp(VKFullUser user) {

        switch (user.online_app) {
            case Indentefiers.FAST_MESSENGER:
                return "Fast Messenger";

            case Indentefiers.KATE_MOBILE:
                return "Kate Mobile";

            case Indentefiers.ANDROID_OFFICIAL:
                return "VK for Android";

            case Indentefiers.AMBERFOG:
                return "VK Amberfog";

            case Indentefiers.EUPHORIA:
                return "Euphoria VK";

            case Indentefiers.IPAD_OFFICIAL:
            case Indentefiers.IPHONE_OFFICIAL:
                return "VK for IOS";

            case Indentefiers.LYNT:
                return "Lynt";

            case Indentefiers.MESSENGER:
            case Indentefiers.PHOENIX:
                return "Phoenix for VK";

            case Indentefiers.ROCKET:
                return "Rocket Messenger";

            case Indentefiers.SWEET:
                return "Sweet";

            case Indentefiers.VK_MD:
                return "VK MD";

            case Indentefiers.WINDOWS_OFFICIAL:
            case Indentefiers.WP_OFFICIAL:
                return "VK for WP";

            case Indentefiers.ZEUS:
                return "Zeus for VK";
        }
        return "приложение";
    }
    ...
    public synchronized static Api init(String access_token, String api_id) {
        if (mSingleton == null) {
    ...
    public ArrayList<VKApp> getApplicationById(int id) throws IOException, JSONException, KException {
        VKParams params = new VKParams("apps.get");
        params.put("id", id);
        params.put("app_ids", "");
        params.put("platform", "android");
        params.put("extended", false);
        params.put("return_friends", false);
        params.put("fields", "");
        params.put("name_case", "");
        addCaptchaParams("", "", params);
        JSONObject root = sendRequest(params);
        JSONArray array = root.optJSONArray("response");
        return VKApp.parseApp(array);
    }

    //*** methods for users ***//
    //http://vk.com/dev/users.get
    public ArrayList<VKFullUser> getProfiles(Collection<Long> uids, Collection<String> domains, String fields, String name_case, String captcha_key, String captcha_sid) throws IOException, JSONException, KException {
    ...
    public VKFullUser getProfile(long user_id) {
        try {
            ...
            ArrayList<VKFullUser> users = getProfiles(uids, null, VKFullUser.DEFAULT_FIELDS, null, null, null);
            return users.get(0);
    ...
    public ArrayList<VKUser> getFriends(long user_id, String order, Integer lid, String captcha_key, String captcha_sid) throws JSONException, IOException, KException {
        VKParams params = new VKParams("friends.get");
        params.put("user_id", user_id);
    ...
    public ArrayList<VKMessageAttachment> getHistoryAttachments(Long peer_id, String media_type, Integer offset, Integer count, Boolean photo_sizes) throws IOException, JSONException, KException {
    ...
    public ArrayList<VKMessage> getMessagesHistory(long uid, long chat_id, long me, long offset, int count) throws IOException, JSONException, KException {
...
//        }
    ...
    public String markAsNewOrAsRead(ArrayList<Long> mids, boolean as_read) throws IOException, JSONException, KException {
        if (mids == null || mids.size() == 0)
            return null;
        VKParams params = new VKParams(as_read ? "messages.markAsRead" : "messages.markAsNew");
    ...
    public NewsFeed getNews(Long start_time, long count, Long end_time, String from, String source_ids, String filters, Integer max_photos, String captcha_key, String captcha_sid) throws IOException, JSONException, KException {
    ...
    public ArrayList<VKNote> getNotes(Long uid, Collection<Long> nids, String sort, Long count, Long offset) throws IOException, JSONException, KException {
        ...
        JSONObject response = root.optJSONObject("response");
        JSONArray array = response.optJSONArray("items");
        ArrayList<VKNote> notes = new ArrayList<VKNote>();
        if (array != null) {
            for (int i = 0; i < array.length(); ++i) {
                JSONObject o = (JSONObject) array.get(i);
                VKNote note = VKNote.parse(o);
                notes.add(note);
            }
        }
        return notes;
    ...
    public VKGroup getGroup(long id) throws JSONException, IOException, KException {
    ...
    public ArrayList<VKFullUser> searchUserExtended(String q, String fields, Long count, Long offset, Integer sort,
                                                    ...
                                                    Integer online, Integer has_photo, Integer school_country, Integer school_city, Integer school, Integer school_year,
                                                    String religion, String interests, String company, String position, Long gid) throws IOException, JSONException, KException {
        String uids = VKUtils.parseProfileId(q);
